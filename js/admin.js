(function () {
  ...
  navLinks.forEach(link => {
    ...
    link.addEventListener('click', async () => {
      ...
      if (link.dataset.panel === 'delivery') {
  ...
  async function fetchDeliveryOrders() {
    ...
    if (deliveryFilter !== 'todos') {
      ...
      query =
        query.eq('estado', deliveryFilter);
  ...
  function renderOrderCard(order) {
    ...
    const items =
      [
        order.principal,
        order.acompanamiento,
        order.extra_ensalada,
        order.bebida
      ].filter(Boolean);


    return `
      <article class="order-card ${isPending ? 'is-pending' : 'is-delivered'}">

        <div class="order-card-header">

          <span class="order-meal-tag">

            <i data-lucide="${order.tipo_comida === 'desayuno' ? 'coffee' : 'utensils'}"></i>

            ${order.tipo_comida === 'desayuno' ? 'Desayuno' : 'Almuerzo'}

          </span>


          <span class="order-status-badge order-status-${order.estado}">
        ...
        <h4>
          ${escapeHtml(order.nombre)}
        </h4>


        <p class="order-detail">
          <i data-lucide="phone"></i> ${escapeHtml(order.telefono)}
        </p>


        <p class="order-detail">
          <i data-lucide="map-pin"></i> ${escapeHtml(order.ubicacion)}
        </p>


        ${order.detalle
        ? `<p class="order-detail order-notes">${escapeHtml(order.detalle)}</p>`
        : ''}


        ${items.length
        ? `<p class="order-items">${escapeHtml(items.join(' · '))}</p>`
        : ''}
        ...
        <div class="order-card-footer">
          ...
          <span class="order-time">
            ${formatOrderDate(order.fecha_pedido)}
            ${order.precio_delivery != null ? ` · Envío: ₡${order.precio_delivery}` : ''}
          </span>


          ${isPending
        ? `<button type="button" class="btn-mark-delivered" data-id="${order.id}">
  ...
  async function renderDeliveryOrders() {
    ...
    deliveryOrdersListEl.innerHTML =
      '<p class="category-empty">Cargando pedidos...</p>';
    ...
    try {
      ...
      deliveryOrdersListEl.innerHTML =
        orders.length
          ...
          : '<p class="category-empty">No hay pedidos en esta categoría.</p>';
      ...
      deliveryOrdersListEl
        .querySelectorAll('.btn-mark-delivered')
        .forEach(btn => {
      ...
      deliveryOrdersListEl.innerHTML =
        '<p class="category-empty">No se pudieron cargar los pedidos.</p>';
